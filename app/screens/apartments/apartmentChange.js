...
export class ApartmentChange extends React.Component {
  ...
  componentDidMount() {
    AsyncStorage.getItem("token_data").then((token_data) => {
      ...
      if(token_data !== null){
        ...
        this.setState({is_trader: user.is_trader, token: token})
        let url = base_url + 'api/apartments/getChangeList?user_id=' + user.id;
  ...
  _apChange(room_id,point){

    this.setState({ isLoading: true })

    let url = base_url + 'api/apartments/ap-change?room_id='
      + room_id + '&user_id=' + this.state.user_id
        ...
        this.props.navigation.dispatch(StackActions.reset({
          ...
          actions: [
            NavigationActions.navigate({
              ...
              params: { apartment_id: data.apartment_id,star: point }
            }),
          ],}))
    ...
    .catch((error) => {
      ...
      Alert.alert('', 'マンション切り替えで例外が発生しました。');
      this.setState({ isLoading: false })
          ...
          <FlatList
            ...
            renderItem={({item}) => {
              ...
              if(item.icon_path === ""){
                ...
                return (
                  <TouchableOpacity onPress={this._apChange.bind(this, item.room_id,item.point)}>
                    <View style={[styles.container,styles.tabBoxListItem]}>
                      ...
                      <View style={styles.content}>
                        <View style={styles.contentHeader}>
                          ...
                          <RkText rkType='header6'>{item.room_foor + 'F ' + item.room_num + '号室'}</RkText>
                        </View>
                        <RkText numberOfLines={2} rkType='' style={{paddingTop: 0, fontSize:14}}>{text}</RkText>
                      </View>
                    </View>
                  </TouchableOpacity>
                )
