...
export class EventEdit extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            default_flg: 2,
            t_datas: [],
            no_img: base_url + 'img/no_img.png',
    ...
    componentDidMount() {
        ...
        AsyncStorage.getItem("token_data").then((token_data) => {
            ...
            let url = base_url + 'api/events/get-params?id=' + this.state.event_id + '&user_id=' + user.id
            fetch( url, {
                method: 'GET',
                headers: {
            ...
            .then((responseJson) => {
                ...
                if(data.message){
                    this.setState({isDefaulReading: false})
                    ...
                    let event_d = data.params['event']
                    let start_dt = event_d['start_dt']
                    let end_dt = event_d['end_dt']
                    let select_cate = categorys_data.indexOf(event_d['category'])
                    this.setState({
                        event_id: event_d['id'],
                        title: event_d['name'],
                        start_dt: this.toLocaleString(new Date(event_d['start_dt'])),
                        end_dt: this.toLocaleString(new Date(event_d['end_dt'])),
                        startDate: new Date(event_d['start_dt']),
                        endDate: new Date(event_d['end_dt']),
                        event_type: event_d['category'],
                        select_event_type: select_cate,
                        event_sub: event_d['sub_category'],
                        content: event_d['content'],
                        trader_names: event_d['trader_names'],
                        member_names: event_d['member_names'],
                        apartment_id: event_d['apartment_id'],
                        icon_image: img_url + event_d['icon_img_path'],
                        icon_type: event_d['icon_type'],
                    })
                    if(event_d['icon_type'] === 1){
                    ...
                    // this.setState({categories: Utils.selectPicker(categorys_data)})
                    ...
                    let image_path = ''
                    if(event_d['image_path'] !== ''){
                        image_path = img_url + event_d['image_path']
                    } else {
                        image_path = this.state.no_img
                    }
                    this.setState({
                        isLoading: false,
                        t_datas: t_datas,
                        ...
                        img_type: "image/jpeg",
                        localUri: image_path,
                        defaultImg: img_url + event_d['image_path'],
    ...
    _send(){
        ...
        if(this.state.title === "") {
          Alert.alert("入力チェックエラー","タイトルを入力して下さい。");
            ...
            let url = base_url + 'api/events/edit';
            fetch( url, {
                method: 'POST',
                body: formData
            })
    ...
    _attachmentImageDelete(){
        
        this.setState({
            image: this.state.no_img,
            localUri: this.state.no_img,
            filename: 'no_img.png',
            img_type: "image/jpeg",
            img_width: 0,
            img_height: 0,
    ...
    _selectEventType = (itemValue, itemIndex) => {
        ...
        this.setState({event_type: itemValue, select_event_type: itemIndex})
        let event_sub = this.state.subCategories[itemIndex]
                ...
                <View style={{flex: 1, backgroundColor: '#fff0d1'}}>
                  <View style={{ paddingBottom: 0, backgroundColor: '#000', flexDirection: 'row', height: 55,paddingTop: 10}}>
                    ...
                    <View style={{flex: 4, alignItems: 'center', marginBottom: 0}}>
                      ...
                      <Text style={{color: '#fff', fontSize: 12}}>案件編集</Text>
                    </View>
                    <View style={{flex: 1, alignItems: 'center', marginBottom: 0}}>
                    </View>
                  </View>
                    <ScrollView style={styles.container}>
                        <View style={styles.content}>
                            ...
                            <View style={{flexDirection: 'row', marginTop: 15, marginBottom: 15}}>
                                <View>
                                    <Image source={{ uri: this.state.image }} style={{ borderRadius: 90,width: 180, height: 180 }} />
                                </View>
                                <View style={{marginTop: 0,marginLeft: 15}}>
                                    ...
                                    <RkButton onPress={this._attachmentImageDelete.bind(this)}
                                        rkType='medium stretch'
                                        contentStyle={{color: '#fff'}} style={styles.del_sc}>
                                        削除
                                    </RkButton>
                                </View>
                            </View>
                                        ...
                                        renderItem={({item}) => {
                                            return <TouchableOpacity style={{
                                                        ...
                                                        <View style={{ flex: 3,alignItems: 'flex-start',justifyContent: 'center'}}>
                                                            {
                                                                item.check ? ( <Text style={{fontWeight: 'bold'}}>{`${item.nick_name}`}</Text>)
