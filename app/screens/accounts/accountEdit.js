...
export class AccountEdit extends React.Component {
    ...
    constructor(props) {
        ...
        account_id = navParams.account_id
        let date = new Date()
        this.state = {
            isVisible1: false,
            date1: date,
    ...
    componentDidMount() {
      ...
      AsyncStorage.getItem("token_data").then((token_data) => {
        ...
        if(token_data !== null){
          ...
          let url = base_url + 'api/accounts/get-edit-params?user_id=' + user.id + '&account_id=' + this.state.account_id
          ...
          .then((responseJson) => {
            ...
            if(data.message){
              ...
              let dt = account.deposits_dt.split('/')
              dateObj = new Date(dt[0], dt[1]-1, dt[2]) 
              ...
              let amount_str = account.amount.toString()
              
                let kanma_number = String(amount_str.replace( '-', '' )).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
              this.setState({
                user_id: user.id,
                ...
                deposits: deposits,
                amount: kanma_number,
                deposits_dt: account.deposits_dt,
                use_comment: account.use_comment,
                account_type: account.account_type,
                deposit_type_id: account.deposit_type_id,
                oldDeposit_type_id: account.deposit_type_id,
                oldDp_type: account.deposit_type,
                account: account,
                date1: dateObj,
                date_view1: account.deposits_dt,
                deposit_title: (account.deposit_type_id === 1)? '入': '出',
                role_id: data.role_id,
                accout_id: account.account_id,
    ...
    _send(){
        ...
        if(this.state.amount === "") {
            ...
            let type_str = '入力された内容で更新しますが宜しいですか？'
            if(this.state.oldDeposit_type_id !== this.state.deposit_type_id) {
                if(this.state.oldDp_type === 0 && this.state.dp_type !== 0){
                    type_str = '入金から出金に変更されますが宜しいですか？'
                } else if(this.state.oldDp_type !== 0 && this.state.dp_type === 0){
                    type_str = '出金から入金に変更されますが宜しいですか？'
                }
            }
            let hiduke=new Date(); 
                ...
                <View style={{flex: 1, backgroundColor: '#fff0d1'}}>
                  <View style={{ paddingBottom: 0, backgroundColor: '#000', flexDirection: 'row', height: 55,paddingTop: 10}}>
                    ...
                    <View style={{flex: 4, alignItems: 'center', marginBottom: 0}}>
                      ...
                      <Text style={{color: '#fff', fontSize: 12}}>口座情報編集</Text>
                    </View>
                    <View style={{flex: 1, alignItems: 'center', marginBottom: 0}}>
                    </View>
                  </View>
                    <ScrollView style={styles.container}>
                        ...
                        <View>
                            ...
                            <View style={{marginTop: 30}} >
                                ...
                                rkType='medium stretch success'
                                contentStyle={{color: '#fff', fontSize: 20}}>
                                更新 
                                </RkButton>
                            </View>
                        </View>
